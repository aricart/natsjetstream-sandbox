...
const JSM = require('../src/jsm').JSM
const opts = require('../src/jsm').opts
...
nc.on('connect', async () => {
  ...
  ok = await jsm.consumers.create('visitor', { durable_name: 'me', deliver_subject: 'visitor' })
  console.log('create consumers', ok)

  info = await jsm.consumers.info('visitor', 'me')
  console.log('info', 'consumers', info.name, 'will deliver to', info.config.deliver_subject)
