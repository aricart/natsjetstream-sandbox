...
nc.on('connect', async () => {
  ...
  if (await jsm.streams.list()) {
    ok = await jsm.streams.delete('worker')
    console.debug('delete worker', ok)
  }

  let ok = await jsm.streams.create('worker', { subjects: ['worker.*'], retention: opts.stream.RetentionPolicy.WorkQueue })
  ...
  // pull consumers
  const pullid = await jsm.consumers.create('worker', { durable_name: 'worker' })
