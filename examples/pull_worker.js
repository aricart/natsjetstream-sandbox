...
const nc = NATS.connect({ payload: NATS.Payload.JSON });
nc.on("err", (err) => {
  console.log(err);
});
...
nc.on("connect", async () => {
  ...
  const streams = await jsm.streams.names();
  ...
  const w = new Worker(jsm, "jobs", { durable_name: "w" });
  const msgs = await w.pullGenerator();
